...
class CrmLead(models.Model):
    ...
    monynha_revenue_model = fields.Selection(
        [
            ("service", "Service"),
            ("product", "Product"),
    ...
    monynha_decision_profile = fields.Selection(
        [
            ("solo", "I do most things myself"),
            ("delegate", "I prefer to delegate execution"),
            ("sales_focus", "I am focused on selling"),
            ("unclear", "I am still figuring it out"),
        ],
        string="Operating Style",
    )
    ...
    monynha_opportunities = fields.Json(compute="_compute_monynha_latest_diagnosis")
    monynha_recommended_action = fields.Char(compute="_compute_monynha_latest_diagnosis")
    ...
    @api.depends(
        "monynha_diagnosis_ids.state",
        "monynha_diagnosis_ids.score",
        "monynha_diagnosis_ids.summary",
        "monynha_diagnosis_ids.opportunities",
        "monynha_diagnosis_ids.recommended_action",
        "monynha_diagnosis_ids.completed_at",
    )
    def _compute_monynha_latest_diagnosis(self):
        for lead in self:
            ...
            lead.monynha_opportunities = latest.opportunities if latest else []
            lead.monynha_recommended_action = latest.recommended_action if latest else False

    def _monynha_discovery_answers(self):
        self.ensure_one()
        return {
            "revenue_model": self.monynha_revenue_model or "",
            "decision_profile": self.monynha_decision_profile or "",
            "struggle": self.monynha_struggle or "",
            "website_url": self.website or "",
            "instagram_url": self.monynha_instagram_url or "",
            "linkedin_url": self.monynha_linkedin_url or "",
        }

    def _monynha_create_diagnosis(self, provider="local_rules"):
        self.ensure_one()
        diagnosis = self.env["monynha.lead.diagnosis"].create(
            {"lead_id": self.id, "provider": provider}
        )
    ...
    def action_monynha_generate_diagnosis(self):
        ...
        return {
            ...
            "res_model": "monynha.lead.diagnosis",
            "res_id": diagnosis.id,
            "view_mode": "form",
            "target": "current",
        }
