...
class MonynhaLeadDiagnosis(models.Model):
    ...
    state = fields.Selection([("pending", "Pending"), ("processing", "Processing"), ("completed", "Completed"), ("failed", "Failed")], default="pending", required=True, index=True)
    ...
    _sql_constraints = [("monynha_public_token_unique", "unique(public_token)", "The public diagnosis token must be unique.")]

    @api.depends("public_token")
    def _compute_public_url(self):
        base_url = self.env["ir.config_parameter"].sudo().get_param("web.base.url")
        for diagnosis in self:
            diagnosis.public_url = f"{base_url}/diagnosis/{diagnosis.public_token}" if diagnosis.public_token else False
    ...
    def _build_summary(self, result):
        self.ensure_one()
        strongest = max((("digital maturity", result["digital_maturity"]), ("automation potential", result["automation_potential"]), ("process clarity", result["process_clarity"]), ("Odoo fit", result["odoo_fit"])), key=lambda item: item[1])[0]
        return f"The discovery shows {strongest} as the strongest current signal. Use the detailed dimensions and signals as a conversation starter, not as an automatic sales decision."
    ...
    def action_process(self):
        for diagnosis in self:
            ...
            if not provider:
                diagnosis.write({"state": "failed", "completed_at": fields.Datetime.now(), "error_message": f"Unknown diagnosis provider: {diagnosis.provider}"})
                continue
            try:
                result = provider()
            except Exception as error:
                _logger.exception("Monynha diagnosis failed for lead %s", diagnosis.lead_id.id)
                diagnosis.write({"state": "failed", "completed_at": fields.Datetime.now(), "error_message": str(error)})
                continue
            diagnosis.write({"state": "completed", "completed_at": fields.Datetime.now(), "model_name": "monynha-local-rules-v1", "score": result["overall"], "digital_maturity": result["digital_maturity"], "automation_potential": result["automation_potential"], "process_clarity": result["process_clarity"], "odoo_fit": result["odoo_fit"], "summary": diagnosis._build_summary(result), "signals": result["signals"], "raw_payload": result})
        return True
